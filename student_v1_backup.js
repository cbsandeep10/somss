...
function disableRows(table_name, row_count){
	row_count = row_count * 2;
	console.log("Table name: " + table_name + " Count: " + row_count);
	$("#" + table_name + " tbody tr td a").each(function() {
		if(row_count == 0)
			return;
		console.log($(this));
		//var child_anchor = $(this).find('a');
		var value = $(this).data("value")
		console.log(value);
		$(this).replaceWith(value);
		row_count--;
		//$(this).find('a').removeClass('editable');
		//$(this).find('a').removeClass('editable-click');
	});
}

var prev_courses_count = 0;
...
function AddCourses(Course_details, Grade_details){
	...
	prev_courses_count = rows.length;
	...
	//Disable these rows so user can't edit
	/*
	$("#coursesTable tbody tr td").each(function() {
		console.log($(this).find('a'));
		var child_anchor = $(this).find('a');
		var value = child_anchor.data("value")
		console.log(child_anchor.data("value"));
		child_anchor.replaceWith(value);
		//$(this).find('a').removeClass('editable');
		//$(this).find('a').removeClass('editable-click');
	});
	*/
	
	disableRows("coursesTable",prev_courses_count);
		...
		if(flag=="true" )
		{
			...
			*/
			...
			$.ajax({
				...
				alert("You have submitted the form which will be auto-reviewed and if everything looks fine, you should receive an e-mail. Please make sure that you receive an email on your ASURITE mail id. If you do not receive a mail in that case you have missed out on something and need to reach out to the Graduate Co-ordinator for further assistance.");
