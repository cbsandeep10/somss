...
//console.log(index);
    ...
    $(document).on("click",".archive",function(evt){
...
<body>
<form id="archive" action="archive.php" method="GET"></form>
<form id="delete" action="delete.php" method="GET"></form>
<form method="POST">
<?php
...
if($current_month < 7){
	...
	$table_name = $current_year.'_'.intval($current_year  + 1); 
            ...
            $sql = "SELECT ASU_ID,First_Name,Second_Name,Adivisory_Committee,Advisor_Firstname,Approved,Main_Advisor_Flag,File_Name,Advisor_Secondname,Advisor_Mail, Submission_date, Director_Comments from ".$table_name; 
    ...
    echo "<div class='CSSTableGenerator'><table><tr><td>Sr.No</td><td>First Name</td><td>Last Name</td><td>ASU ID</td><td>Application Submission Date</td><td>ADVISORS PRESENT</td><td>Student IPOS</td><td>Advising Committee Approval</td><td>Actions</td><td>Director Approval</td></tr>";
	$current_year = date('Y');
$current_month = date('m');
					if($current_month<7)
							{
    ...
    {
    $advisor_formed="YES";
    echo 'console.log("hello1")';
    echo"
     
    <tr>
    <td name='srno' id='srno'>".$count."</td>
    <td>{$row[1]}</td><td>{$row[2]}</td><td >
    <a href=student_demo.php?ID=$row[0]&AcademicYear=$current_academic_year name='ID' id='ID_".$count."' value=".$row[0].">{$row[0]}</a></td><td>{$row[10]}</td><td>$advisor_formed</td><td><a href=./uploads/$row[7]>IPOS</a></td><td><div class='approval' id='approval$count' title='The form has not yet been approved by any member of the Advising Committee'></div></td><td>
    <script>
    if(".$total_count."!=".$total_approved."){
       document.write(\"<button name='submit_btn' type='submit' class='submit' id='".$count."' value=".$row[0].">Reminder</button>\");
       console.log('".$row[0]."');
       document.write(\"<button name='archive_btn' class='archive' id='sth' form='archive' value=".$row[0].">archive</button>\");
       console.log('".$row[0]."');
       document.write(\"<button name='delete_btn' class='delete' id='del' form='delete' value=".$row[0].">delete</button>\");    
	}
...
$director_comments
    ...
    echo"<tr>
    <td name='srno' id='srno'>".$count."</td>
    <td>{$row[1]}</td><td>{$row[2]}</td><td><a href=student_demo.php?ID=$row[0]&AcademicYear=$current_academic_year>{$row[0]}</a></td><td>{$row[10]}</td><td>$advisor_formed</td><td><a href=./uploads/$row[7]>IPOS</a></td><td><div class='approval' id='approval$count' title='The student does not have a Advising Committee'></div></td><td></td><td>$director_comments</td></tr>";
    }
